...
def test_mock(mocker: MockerFixture):
    ...
    _read: mock.MagicMock = mocker.patch("gopher.Input", wraps=judge.output)
    write: mock.MagicMock = mocker.patch("gopher.Output", wraps=judge.input)

    try:
        gopher.solve_case(...)
    except gopher.EndInteractive:
        ...
...
def test_wrong_answer(mocker: MockerFixture):
    ...
    read.side_effect = [gopher.WRONG_ANSWER]

    with pytest.raises(gopher.EndInteractive):
        gopher.solve_case(...)
...
class FakeIO:
    def __init__(self, mocker: MockerFixture, timeout: Optional[int] = 3):
        ...
        self.sol_read = mocker.patch("gopher.sys.stdin.readline", wraps=Input(self.judge_sol_queue, timeout))  # type: ignore
        self.sol_write = mocker.patch("gopher.sys.stdout.write", wraps=Output(self.sol_judge_queue))  # type: ignore
...
def test_threads_main(mocker: MockerFixture):
    ...
    args = [""]
    mocker.patch.object(gopher_judge.sys, "argv", args)
    mocker.patch("gopher_judge.sys.exit")

    thread = threading.Thread(target=gopher_judge.main)
    thread.start()
    ...
    assert IO.judge_write.mock_calls[-1] != mocker.call(gopher.WRONG_ANSWER)
...
def test_threads_runcases(mocker: MockerFixture):
    ...
    thread = threading.Thread(target=gopher_judge.RunCases, args=args)
    thread.start()

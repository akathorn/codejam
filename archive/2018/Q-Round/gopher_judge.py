    ...
    for test_case_id in range(1, NUM_TEST_CASES + 1):
        ...
        for _ in range(INTERACTIONS):
            ...
            if not (prepared_i, prepared_j) in field:
                ...
                if (prepared_cells_count >= A) and (
                    prepared_cells_count
                    == (southmost - northmost + 1) * (eastmost - westmost + 1)
                ):
                    JudgePrint(p, "0 0")
        ...
        if not test_case_passed:
            WrongAnswerExit(
                p, test_case_id, "Failed to prepare the rectangle within 1000 tries."
            )
